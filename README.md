## 在线游玩

[打开 GitHub Pages 版猫猫盘串铺](https://ziyuezhou1.github.io/CAT-BODHI/)

<div align="center">

```
╔══════════════════════════════════════════════════════╗
║                                                      ║
║   ▓▄▄▄▄  ▓▄▄▄  ▄▄▄▄▄▄    ▓▄▄▄▄  ▓▄▄▄  ▓▄▄▄  ▄  ▄  ║
║   ██  ██ ██  ██ ██  ██ ██ ██  ██ ██  ██ ██  ██ ██▄▀█  ║
║   ██▄▄██ ██▄▄██ ██  ██ ██ ██▄▄██ ██▄▄██ ██▄▄██ ██ ▀█  ║
║   ██     ██  ██ ██  ██    ██  ██ ██  ██ ██  ██ ██  █  ║
║                                                      ║
║         猫猫盘串铺 · CAT BODHI                        ║
║         像素风文玩放置游戏                              ║
╚══════════════════════════════════════════════════════╝
```

</div>

---

## 更新内容

### 2026-05-28

- 重做 AI 形象工坊流程：游戏内复制稳定 sprite 生图提示词，玩家到千问、豆包、OpenAI、Gemini 等外部模型自行生成图片。
- 猫猫提示词要求生成纯绿色背景 `#04F90E` 的横向三动作 sprite sheet：坐姿、跳跃、趴卧，同一只参考猫不变形；文玩/珠串提示词也强制使用同样的纯绿色抠图背景。
- 上传外部生成的猫猫/文玩 sprite 后，本地服务会调用 `D:\sprite_alpha_seg_pytorch` 的 PyTorch 分割模型抠背景、裁切；猫猫会按三帧动作分别加入游戏存档。
- 新增 `/api/sprite-import` 本地导入接口，以及 `tools/process_sprite_upload.py` 处理脚本。
- 猫缘和珠阶面板移除 API Key 与模型选择输入，改成复制提示词、上传 sprite、取名入档的流程。
- README 补充本地 sprite 处理模型路径配置。
- 修复微信内置浏览器无法下载存档文件的问题：微信中导出会显示可复制的存档文本。
- 修复领悟福爪后自定义猫猫/珠串被清空的问题；重新开铺会保留玩家上传的形象与已有自定义收藏。
- 修复自定义猫猫/珠串跨浏览器存档迁移：导出存档会内嵌玩家上传形象的图片数据，导入后不再依赖本机图片路径。
- 修复手机访问本地抠图服务的指引和接口判断：手机需打开电脑局域网地址 `http://电脑IP:8080`，不能从 GitHub Pages 直接调用电脑本地模型。
- AI 工坊新增深度学习服务地址与检测入口；可连接本机、局域网电脑，或未来部署的 HTTPS 云端抠图服务。
- 仓库内置最小 `sprite_alpha_seg_pytorch` 推理包与 `unet_sprite_ft.pt` 权重，部署方无需再寻找外部私有模型仓库。

---

## 🐾 关于游戏

欢迎来到 **猫猫盘串铺**，一款像素风格的文玩盘串放置游戏。

在小猫们的陪伴下，盘动手中的菩提手串，积攒 **禅意**，解锁更珍稀的珠串，装点你的猫猫小屋，达成灵修心愿。

---

## 🎮 玩法

| 操作 | 说明 |
|------|------|
| 🖱️ **盘串** | 点击中央的菩提台，每次点击获得禅意，手串在指尖旋转 |
| 🐱 **猫缘** | 招募猫猫伙伴，它们会持续为你产出 **猫息**（被动收入） |
| 📿 **珠阶** | 积攒足够禅意后解锁更高级的手串 — 菩提根 → 猴头 → 星月菩提 → 小金刚 |
| 🪴 **装饰** | 购买猫爬架、猫窝、抓抓柱等装饰，提升各项产出倍率 |
| ⭐ **心愿** | 完成阶段性目标，获得 **福爪** 永久加成 |

---

## 🐈 猫猫图鉴

| 猫猫 | 名称 | 解锁 | 特点 |
|------|------|------|------|
| 🟠 | **橘串师** | 初始 | 勤勤恳恳的老橘猫，盘串手艺代代相传 |
| ⚪ | **白团守垫** | 550 禅意 | 爱趴在猫窝上的白团子，软乎乎但很可靠 |
| ⚫ | **黑禅猫** | 5200 禅意 | 双目微阖的禅修猫，猫息深邃悠长 |
| 🟡 | **三花掌柜** | 46000 禅意 | 文玩铺子的话事猫，三花色才是正统 |

---

## 📿 手串进阶

| 手串 | 解锁禅意 | 倍率 |
|------|----------|------|
| 菩提根 | 初始 | ×1.0 |
| 猴头 | 1.5 万 | ×1.45 |
| 星月菩提 | 18 万 | ×2.1 |
| 小金刚 | 250 万 | ×3.2 |

---

## 🛠️ 技术栈

纯原生前端三件套，零依赖，打开即玩：

- **HTML5** — 语义化结构，无障碍支持
- **CSS3** — 像素风视觉系统，CSS 精灵动画，响应式布局
- **Vanilla JS** — 离线收益计算，LocalStorage 自动存档

---

## 🚀 快速开始

```bash
# 任意静态服务器即可
npx serve .
# 或
python -m http.server 8080
```

然后打开 `http://localhost:8080`

### AI 形象工坊

猫缘页和珠阶页现在使用“外部模型生成 + 深度学习抠图切割”的流程：

1. 在游戏里点击“复制猫猫提示词”或“复制手串提示词”。
2. 到自己的千问、豆包、OpenAI、Gemini 等模型中粘贴提示词，并附上现实猫照或文玩照片。猫猫请生成纯绿色背景的横向三动作 sprite sheet；文玩/珠串也请生成纯绿色背景。提示词会要求纯色 `#04F90E`，并禁止渐变背景、水印、logo 和“豆包AI生成”等平台字样。
3. 把外部模型生成的 sprite 图片上传回游戏，填写名字。
4. 游戏默认调用 `https://cat-bodhi.xiteng.site` 的 PyTorch 分割服务；也可以在“抠图服务地址”中改成本机或其他云端服务。服务会把处理后的 PNG 加入猫缘或珠串存档。

自定义加入的猫咪和珠串会在对应卡片右上角显示删除按钮。删除猫咪会清掉它的数量、状态和场景摆放；删除自定义珠串会清掉该珠阶下的收藏串，并自动切回仍可用的主串。

处理器会优先识别从画布边缘连通进来的绿色/青绿色背景，对豆包等模型偶尔生成的薄荷绿渐变底、轻微压缩噪点和边缘色溢出做容错；但为了最干净的像素边缘，仍建议外部模型输出纯色背景且不要带水印。

启动本地服务：

```bash
npm run dev:ai
```

电脑上打开 `http://localhost:8080`。

Windows 也可以双击 `start-local-ai.bat` 启动本地深度学习服务。

手机上使用本地抠图切割：电脑和手机连同一个 Wi-Fi，先在电脑运行 `npm run dev:ai`，然后看终端打印的 `Phone/LAN URL`，在手机浏览器或微信里打开类似 `http://192.168.1.23:8080` 的地址。不要用 GitHub Pages 页面直接调用本地模型，因为手机里的 `localhost` 指的是手机自己，HTTPS 页面也通常不能调用局域网 HTTP 服务。

云端默认服务地址是 `https://cat-bodhi.xiteng.site`。如果模型部署到其他云服务器，服务只要兼容 `/api/sprite-status` 与 `/api/sprite-import`，就可以在游戏里的“抠图服务地址”填入 HTTPS 地址后检测并调用。

默认模型路径：

```env
SPRITE_SEG_ROOT=./sprite_alpha_seg_pytorch
SPRITE_SEG_PYTHON=python
SPRITE_SEG_CHECKPOINT=./sprite_alpha_seg_pytorch/checkpoints/unet_sprite_ft.pt
SPRITE_SEG_OUT_DIR=./sprite_alpha_seg_pytorch/outputs/cat_match
```

Windows 本机如果存在 `D:\sprite_alpha_seg_pytorch`，服务会优先沿用该目录和其中的 `.venv`；服务器部署时默认使用仓库内置的 `sprite_alpha_seg_pytorch`。仓库没有打包 Python 虚拟环境和 PyTorch wheel，部署端需要按 `sprite_alpha_seg_pytorch/requirements.txt` 安装依赖。

如果你的模型项目放在别处，可以在启动前用环境变量覆盖这些路径。

---

## 📂 项目结构

```
CAT-BODHI/
├── index.html          # 游戏入口
├── server.mjs          # 本地 sprite 导入处理服务与静态服务器
├── game.js             # 核心逻辑 (状态管理 / 数值系统 / UI 渲染)
├── styles.css          # 像素风视觉系统
└── assets/
    └── art/            # 像素精灵图 (猫猫 / 手串 / 装饰 / UI)
```

---

<div align="center">

```
  猫猫盘串，禅意自来  ✧  菩提在手，福爪随身
```

</div>
