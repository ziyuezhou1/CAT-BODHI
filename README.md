...
- 猫猫提示词要求生成纯洋红色背景 `#FF00FF` 的横向三动作 sprite sheet：坐姿、跳跃、趴卧，同一只参考猫不变形；文玩/珠串提示词也使用同样的纯洋红色抠图背景。
...
2. 到自己的千问、豆包、OpenAI、Gemini 等模型中粘贴提示词，并附上现实猫照或文玩照片。猫猫请生成纯洋红色背景的横向三动作 sprite sheet；文玩/珠串也请生成纯洋红色背景。提示词会要求纯色 `#FF00FF`，并禁止渐变背景、水印、logo 和“豆包AI生成”等平台字样。
...
自定义加入的猫咪和珠串会在对应卡片右上角显示删除按钮。删除猫咪会清掉它的数量、状态和场景摆放；删除自定义珠串会清掉该珠阶下的收藏串，并自动切回仍可用的主串。

处理器会从画布角落识别实际背景色，并结合 RGB、HSV 与 Lab 色彩信息处理色差、压缩噪点和边缘溢色；闭环手串中央与背景匹配的区域也会单独清除。为了获得最干净的像素边缘，建议外部模型输出纯洋红色 `#FF00FF` 背景，不要使用渐变、纹理或水印。
...
Windows 也可以双击 `start-local-ai.bat` 启动本地深度学习服务。
